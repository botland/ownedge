...
@pytest.mark.acceptance
@pytest.mark.asyncio
async def test_ac2_last_reconcile_ts_updates(fresh_state, serving_backend):
    """AC2: last_reconcile_ts updates during reconcile."""
    reconciler = Reconciler("ac-appliance", serving_backend=serving_backend)
...
@pytest.mark.acceptance
@pytest.mark.asyncio
async def test_ac6_cache_disk_error_degraded(fresh_state, serving_backend):
...
@pytest.mark.acceptance
@pytest.mark.asyncio
async def test_ac7_cpu_only_degraded_not_failed(fresh_state, serving_backend):
...
def test_ac8_single_vllm_container_labels(mock_docker_client, fake_vllm_container, sample_desired):
    ...
    assert labels["inferedge.config_hash"] == compute_config_hash(sample_desired)
...
async def test_ac4_docker_restart_triggers_recreation(
    fresh_state, serving_backend, sample_desired, patch_reconcile_externals
):
    ...
    with (
        ...
        patch.object(serving_backend, "stop_if_needed", AsyncMock(return_value=1)) as stop_mock,
        patch.object(serving_backend, "start_or_update", AsyncMock(return_value="new-cid")) as start_mock,
...
async def test_ac10_reconcile_log_no_growth_on_ready_noop(
    ...
    with patch.object(serving_backend, "get_deployment_status", AsyncMock(return_value=healthy)):
        for _ in range(5):
            await reconciler.reconcile_once()
